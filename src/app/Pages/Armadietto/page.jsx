...
export default function Inventario({ isAuthenticated: initialAuth = false }) {
  ...
  // Stato Modale Aggiungi Farmaco
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [formData, setFormData] = useState({
    nome: "", principio: "", forma: "compresse", dosaggio: "", quantita: "", scadenza: "", aic: ""
  });
  ...
  const handleAddMedicine = () => {
    ...
    const newMed = {
        id_farmaco_armadietto: Date.now(),
        quantita_rimanente: parseInt(formData.quantita),
        data_scadenza: formData.scadenza,
        farmaco: {
            denominazione: formData.nome,
            principio_attivo: formData.principio,
            forma: formData.forma,
            quantita_confezione: parseInt(formData.quantita) // Assumiamo scatola nuova
        },
        computedStatus: 'ok' // Default
    };
    ...
    setStats({
        ...stats,
        total: updatedMedicines.length
    });

    setIsAddDialogOpen(false);
    setFormData({ nome: "", principio: "", forma: "compresse", dosaggio: "", quantita: "", scadenza: "", aic: "" });
  };
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
            ...
            <div className="flex gap-3">
              {/* PULSANTE MODIFICATO PER APRIRE IL POPUP */}
          ...
          {isLoading ? (
              ...
              {filteredMedicines.map((medicine, index) => {
                ...
                return (
                  <div key={uniqueId} className="group relative bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-all hover:border-teal-100">
                    <Link href={`/farmaco/${uniqueId}`} className="absolute inset-0 z-0" />
                    ...
                    {openDropdownId === uniqueId && (
                      <div className="absolute right-3 top-12 w-44 bg-white rounded-lg shadow-lg border border-slate-100 py-1.5 z-30 animate-in fade-in zoom-in-95 duration-100">
                          <Link href={`/farmaco/${uniqueId}`} className="flex items-center px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-[#14b8a6]"><Icons.Edit className="w-4 h-4 mr-2" /> Modifica</Link>
                          <Link href={`/nuova-terapia?farmaco=${uniqueId}`} className="flex items-center px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-[#14b8a6]"><Icons.Calendar className="w-4 h-4 mr-2" /> Terapia</Link>
                          <div className="h-px bg-slate-100 my-1"></div>
                          <button className="w-full flex items-center px-3 py-2 text-sm text-rose-600 hover:bg-rose-50 font-medium"><Icons.Trash2 className="w-4 h-4 mr-2" /> Elimina</button>
                      </div>
                    )}
                  </div>
                );
            ...
            <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
              ...
              <Button onClick={() => setIsAddDialogOpen(true)}>Aggiungi il primo farmaco</Button>
            </div>
          )}
        </div>
      </main>

      {/* --- MODALE POPUP AGGIUNTA FARMACO --- */}
      <Modal 
        ...
        footer={
           <>
              <Button variant="secondary" onClick={() => setIsAddDialogOpen(false)}>Annulla</Button>
              <Button onClick={handleAddMedicine} disabled={!formData.nome || !formData.quantita}>Salva Farmaco</Button>
           </>
        }
      >
        <div className="space-y-4">
           ...
           <div>
              ...
              <input type="text" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#14b8a6]" 
                     placeholder="Es. Tachipirina" value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} />
           </div>
           
           {/* Principio e Dosaggio */}
           <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Principio Attivo</label>
                <input type="text" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#14b8a6]" 
                      placeholder="Es. Paracetamolo" value={formData.principio} onChange={e => setFormData({...formData, principio: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Dosaggio</label>
                <input type="text" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#14b8a6]" 
                      placeholder="Es. 1000mg" value={formData.dosaggio} onChange={e => setFormData({...formData, dosaggio: e.target.value})} />
              </div>
           </div>

           {/* Forma */}
           <div>
               <label className="block text-sm font-semibold text-slate-700 mb-1">Forma Farmaceutica</label>
               <select className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#14b8a6] bg-white"
                       value={formData.forma} onChange={e => setFormData({...formData, forma: e.target.value})}>
           ...
           <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#14b8a6]" 
                       placeholder="Es. 20" value={formData.quantita} onChange={e => setFormData({...formData, quantita: e.target.value})} />
              </div>
              <div>
           ...
           {/* AIC */}
           <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Codice AIC (Opzionale)</label>
              <input type="text" className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#14b8a6]" 
                     placeholder="Es. 012745046" value={formData.aic} onChange={e => setFormData({...formData, aic: e.target.value})} />
           </div>
        </div>
      </Modal>
